...
#textを音素に
def mozi2phone(wavlm, wav_path, d_):
    filename = os.path.basename(wav_path)
    d = os.path.basename(d_)
...
#filelistの1行を作成する
def create_data_line(wav, d, wavlm):
    units = mozi2phone(wavlm, wav, d)
    #wav_path | unit
    one_line = "{}|{}\n".format(
        wav,
        units,
...
def create_dataset(wavlm, filename):
    #空白話者の先頭からスタート
    ...
    for d in textful_dir_list:
        ...
        for wav in wav_file_list:
            one_line = create_data_line(wav, d, wavlm)
...
def main():
    ...
    if args.multi_target != None:
        ...
        n_spk = create_dataset(cmodel, filename)
